...
Board::Board(int columns, int lines, int cell_x, int cell_y)
    : _columns(columns), _lines(lines), _cellX(cell_x), _cellY(cell_y),
      _isGameOver(false), _gridContent(nullptr) {
  ...
  for (int i = 0; i <= columns; ++i) {
    ...
    _grid[(_lines + 1) * 2 + i * 2 + 1] =
        sf::Vertex(sf::Vector2f(i * _cellX, _lines * _cellY));
    ...
    for (int x = 0; x < _columns; ++x) {
      if (_gridContent[y * _columns + x] != CELL_EMPTY) {
        ...
        rectangle.setFillColor(
            Piece::Tetrimino_colors[_gridContent[y * _columns + x]]);
...
bool Board::isFallen(const Piece &piece) {
  ...
  flood(piece.getPosX(), piece.getPosY() + 1, (int)Piece::PIVOT_X,
        (int)Piece::PIVOT_Y, piece.getType(), piece.getRotation(), visited,
        fallen);
...
void Board::flood(int grid_x, int grid_y, int piece_x, int piece_y,
                  Piece::Tetrimino_Types type, int rotation,
                  bool visited[][Piece::MATRIX_SIZE], int value) {
  // limit tests
  if (piece_x < 0 or piece_x >= Piece::MATRIX_SIZE or piece_y < 0 or
      piece_y >= Piece::MATRIX_SIZE or grid_x < 0 or grid_x >= (int)_columns or
      grid_y < 0 or grid_y >= (int)_lines or
      visited[piece_y][piece_x] == true or
      Piece::Tetrimino_Pieces[type][rotation][piece_y][piece_x] == 0)
    return;
  ...
  flood(grid_x, grid_y - 1, piece_x, piece_y - 1, type, rotation, visited,
        value);
  flood(grid_x + 1, grid_y, piece_x + 1, piece_y, type, rotation, visited,
        value);
  flood(grid_x, grid_y + 1, piece_x, piece_y + 1, type, rotation, visited,
        value);
  flood(grid_x - 1, grid_y, piece_x - 1, piece_y, type, rotation, visited,
...
void Board::flood(int grid_x, int grid_y, int piece_x, int piece_y,
  ...
  // limit tests
  if (piece_x < 0 or piece_x >= Piece::MATRIX_SIZE or piece_y < 0 or
      piece_y >= Piece::MATRIX_SIZE or visited[piece_y][piece_x] == true or
      Piece::Tetrimino_Pieces[type][rotation][piece_y][piece_x] == 0)
    return;
  ...
  // collision tests
  if (grid_x < 0 or grid_x >= (int)_columns or grid_y < 0 or
      grid_y >= (int)_lines or
